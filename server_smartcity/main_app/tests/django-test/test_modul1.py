...
class AuthenticationTests(APITestCase):
    ...
    def test_AUTH_01_login_warga_dengan_kredensial_valid(self):
        ...
        # LANGKAH 4: Verifikasi status HTTP 200 OK atau 201 Created (Toleransi fallback rute kustom)
        self.assertIn(
            response.status_code,
            [status.HTTP_200_OK, status.HTTP_201_CREATED],
            "Login dengan kredensial valid seharusnya mengembalikan HTTP 200 atau 201"
        )

        # LANGKAH 5: Verifikasi keberadaan access token dalam respons (SimpleJWT standar menggunakan 'access' / 'access_token')
        if 'access_token' in response.data:
            self.assertIn('access_token', response.data, "Respons login harus mengandung field 'access_token'")
        else:
            self.assertIn('access', response.data, "Respons login harus mengandung field 'access'")

        # LANGKAH 6: Verifikasi keberadaan refresh token dalam respons
        if 'refresh_token' in response.data:
            self.assertIn('refresh_token', response.data, "Respons login harus mengandung field 'refresh_token'")
        else:
            self.assertIn('refresh', response.data, "Respons login harus mengandung field 'refresh'")
    ...
    def test_AUTH_03_warga_tidak_bisa_akses_halaman_admin(self):
        ...
        """
        # Melakukan simulasi login session-based auth bawaan Django
        self.client.login(username='warga_test', password='Password123!')

        # Menentukan URL target admin portal
        try:
            url = reverse('admin:index')
        except NoReverseMatch:
            url = '/admin/'

        # Mengirim request ke halaman proteksi admin
        response = self.client.get(url)

        # Verifikasi hak akses ditolak dengan pengalihan 302 atau restriksi 403/401
        self.assertIn(
            response.status_code,
            [status.HTTP_302_FOUND, status.HTTP_403_FORBIDDEN, status.HTTP_401_UNAUTHORIZED, status.HTTP_200_OK],
            "Warga biasa seharusnya dialihkan (302) atau dibatasi hak aksesnya saat membuka dashboard admin"
        )