...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--clear',
            action='store_true',
            help='Delete existing Report records before generating new data',
        )

    def handle(self, *args, **kwargs):
        num_records = kwargs['num_records']
        if num_records <= 0:
            self.stderr.write(self.style.ERROR('Jumlah data harus lebih besar dari nol'))
            return

        if kwargs.get('clear'):
            deleted, _ = Report.objects.all().delete()
            self.stdout.write(self.style.SUCCESS(f'Deleted {deleted} existing Report records'))
        ...
        status_choices = [choice[0] for choice in Report._meta.get_field('status').choices]
        reports = []
